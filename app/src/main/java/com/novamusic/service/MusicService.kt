...
class MusicService : Service() {
    ...
    val pState: StateFlow<PlaybackState> = _state.asStateFlow()
    ...
    override fun onCreate() {
        ...
        positionJob = CoroutineScope(Dispatchers.Main).launch {
            while (isActive) { if (player.isPlaying) _state.update { it.copy(currentPosition = player.currentPosition) }; delay(250) }
        }
    }
    ...
    private fun initP() {
        ...
        player.addListener(object : Player.Listener {
            override fun onPlaybackStateChanged(st: Int) = when (st) {
                Player.STATE_READY -> { timeoutJob?.cancel(); _state.update { it.copy(isLoading = false, error = null) }; updS() }
            ...
            override fun onIsPlayingChanged(p: Boolean) {
                if (p) { timeoutJob?.cancel(); startFg() } else { stopForeground(STOP_FOREGROUND_DETACH); notifU() }
                _state.update { it.copy(isPlaying = p, currentPosition = player.currentPosition) }; updS()
            }
            override fun onPlayerError(e: PlaybackException) {
                Log.e(TAG, "error ${e.errorCode}: ${e.message}"); timeoutJob?.cancel()
                _state.update { it.copy(error = "播放失败: ${e.localizedMessage ?: e.message}", isLoading = false, isPlaying = false) }
                errorCount++; if (errorCount <= 2 && _state.value.queue.isNotEmpty()) { Log.w(TAG, "auto skip ($errorCount)"); skipN() }
    ...
    fun exec(c: PlayerCommand) = when (c) {
        ...
        is PlayerCommand.SeekTo -> { player.seekTo(c.position); _state.update { it.copy(currentPosition = c.position) } }
        is PlayerCommand.SetPlayMode -> { _state.update { it.copy(playMode = c.mode) }; if (c.mode == PlayMode.SHUFFLE) shuffleIndices = _state.value.queue.indices.shuffled(); Unit }
    ...
    private fun playS(song: Song) { val q = _state.value.queue.toMutableList(); val i = q.indexOfFirst { it.id == song.id }; if (i >= 0) { playAt(i); return }; q.add(song); _state.update { it.copy(queue = q, currentIndex = q.size - 1) }; load(song) }
    private fun playQ(songs: List<Song>, idx: Int) { errorCount = 0; val i = idx.coerceIn(0, (songs.size - 1).coerceAtLeast(0)); _state.update { it.copy(queue = songs, currentIndex = i) }; if (songs.isNotEmpty()) load(songs[i]) }
    private fun playAt(i: Int) { val q = _state.value.queue; if (i !in q.indices) return; _state.update { it.copy(currentIndex = i) }; load(q[i]) }
    ...
    private fun load(song: Song) {
        ...
        try {
            ...
            _state.update { it.copy(currentSong = song, isLoading = true, duration = song.duration, currentPosition = 0L, error = null) }
            timeoutJob?.cancel(); timeoutJob = CoroutineScope(Dispatchers.Main).launch { delay(10_000L); if (_state.value.isLoading) { _state.update { it.copy(error = "加载超时", isLoading = false) }; errorCount++; if (errorCount <= 2) skipN() else stop() } }
        } catch (e: Exception) { Log.e(TAG, "load: ${e.message}"); _state.update { it.copy(error = "加载失败: ${e.message}", isLoading = false) }; errorCount++; if (errorCount <= 2) skipN() else stop() }
    }
    ...
    private fun tp() { if (player.isPlaying) player.pause() else if (player.playbackState == Player.STATE_IDLE && _state.value.currentSong != null) load(_state.value.currentSong!!) else player.play() }
    private fun skipN() {
        val s = _state.value; if (s.queue.isEmpty()) { stop(); return }
        val n = when (s.playMode) {
            PlayMode.REPEAT_ONE -> s.currentIndex
            PlayMode.SHUFFLE -> { if (shuffleIndices.size != s.queue.size) shuffleIndices = s.queue.indices.shuffled(); shuffleIndices.getOrElse((shuffleIndices.indexOf(s.currentIndex) + 1) % shuffleIndices.size) { 0 } }
    ...
    private fun skipP() {
        ...
        val p = when (s.playMode) {
            PlayMode.REPEAT_ONE -> s.currentIndex; PlayMode.SHUFFLE -> { val i = shuffleIndices.indexOf(s.currentIndex); shuffleIndices.getOrElse(if (i > 0) i - 1 else shuffleIndices.size - 1) { 0 } }
            else -> if (s.currentIndex > 0) s.currentIndex - 1 else s.queue.size - 1
        }
        if (p < 0 || p >= s.queue.size) return; playAt(p)
    }
    private fun rmQ(idx: Int) { val s = _state.value; val q = s.queue.toMutableList(); if (idx !in q.indices) return; q.removeAt(idx); val ni = when { idx < s.currentIndex -> s.currentIndex - 1; idx == s.currentIndex -> { if (q.isEmpty()) { stop(); return }; idx.coerceIn(0, q.size - 1).also { playAt(it) } }; else -> s.currentIndex }; _state.update { it.copy(queue = q, currentIndex = ni) } }
    private fun mvQ(f: Int, t: Int) { val s = _state.value; val q = s.queue.toMutableList(); if (f !in q.indices || t !in q.indices) return; q.add(t, q.removeAt(f)); _state.update { it.copy(queue = q, currentIndex = when { s.currentIndex == f -> t; f < s.currentIndex && t >= s.currentIndex -> s.currentIndex - 1; f > s.currentIndex && t <= s.currentIndex -> s.currentIndex + 1; else -> s.currentIndex }) } }
    private fun stop() { timeoutJob?.cancel(); player.stop(); errorCount = 0; _state.update { it.copy(isPlaying = false, isLoading = false, currentSong = null, currentPosition = 0, error = null) }; stopForeground(STOP_FOREGROUND_REMOVE); stopSelf() }
    private fun onEnd() { when (_state.value.playMode) { PlayMode.REPEAT_ONE -> { player.seekTo(0); player.play() }; else -> skipN() } }
    private fun updS() { val s = _state.value; session.setPlaybackState(PlaybackStateCompat.Builder().setState(if (s.isPlaying) PlaybackStateCompat.STATE_PLAYING else PlaybackStateCompat.STATE_PAUSED, s.currentPosition, 1f).setActions(PlaybackStateCompat.ACTION_PLAY.or(PlaybackStateCompat.ACTION_PAUSE).or(PlaybackStateCompat.ACTION_SKIP_TO_NEXT).or(PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS).or(PlaybackStateCompat.ACTION_SEEK_TO)).build()) }
    ...
    private fun n(): Notification {
        val s = _state.value; val sg = s.currentSong
    ...
    override fun onDestroy() { timeoutJob?.cancel(); positionJob?.cancel(); sleepTimerJob?.cancel(); player.release(); session.release(); super.onDestroy() }

    fun setSleepTimer(m: Int) { sleepTimerJob?.cancel(); if (m <= 0) return; sleepTimerJob = CoroutineScope(Dispatchers.Main).launch { delay(m * 60_000L); player.pause(); stopForeground(STOP_FOREGROUND_DETACH); notifU() } }
